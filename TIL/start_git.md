...
- `git log --oneline`: 깃 로그를 간략하게 한눈에 볼 수 있도록 출력해줌
...
- `git merge 병합할 브랜치 이름`
  - merge하기 전에 일단 다른 브랜치를 합치려고 하는 즉 메인 브랜치로 switch해야 함
- 병합된 브랜치는 삭제를 해주면서 나아가기

### 1. fast-foward

- 마스터가 최신버전쪽으로 나아가는 것(브랜치쪽으로)
- 병합을 하는 것은 맞지만 실제 동작은 단순히 헤드가 앞으로 나갈 뿐임 그래서 fast-foward
- 브랜치를 병합할 때 마스터는 이전에 브랜치가 만들어질 때와 변화없이 똑같을 때 병합방법

### 2.  3-way merge (merge commit)

- 브랜치가 갈라진 이후 마스터에도 변화가 생겼을 때
- 병합하려는 브랜치와 변화가 생긴 마스터의 공통된 조상하나를 사용해서 병합을 진행하기때문에 3way임, 새로운 버전의 마스터가 생기기 때문에 merge commit이라고 함

### 3. merge conflict

- 병합하는 두 브랜치에서 같은 파일의 같은 부분을 동시에 수정하고 병합하면 깃은 해당 부분을 자동으로 병합해주지 못함
- 반면 동일 파일이더라도 서로 다른 부분을 수정했다면 conflict없이 자동으로 머지커밋 됨
- 이렇게 하면 원래 파일에 여러 브랜치에서 동시에 작성한 것들이 구역이 나뉘어서 보여지는데 이것들을 다 지우고 남길 것만 남기고 커밋하면 됨
